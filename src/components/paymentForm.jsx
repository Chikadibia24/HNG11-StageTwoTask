...
const PaymentForm = ({onClick}) => {
  ...
  const PaymentButton = ({buttonClassName,icon,text,topIconClassName,topIcon}) => {
    return (
      <button
        className={`flex items-center justify-center gap-[5px] md:gap-[8px] py-[15px] w-[118px] px-[6px] md:px-[10px] rounded-[4px] lg:h-[38px] lg:py-[16px] ${buttonClassName}`}
      >
        <span>
          <img src={icon} alt={icon} />
        </span>

        <span className="text-[9px] md:text-[14px] lg:text-[15px] text-[#404040]">{text}</span>
        <div
